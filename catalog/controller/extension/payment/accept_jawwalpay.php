...
class ControllerExtensionPaymentAcceptJawwalpay extends Controller
{
    ...
    public function index()
    {
        ...
        $this->load->model('extension/payment/accept_jawwalpay');
        $helper = $this->model_extension_payment_accept_jawwalpay;

        $debug              = $this->config->get('payment_accept_jawwalpay_allow_debug');
        $order              = $this->model_checkout_order->getOrder($this->session->data['order_id']);
        $customer           = $this->model_account_customer->getCustomerByEmail($order['email']);
        $acceptToken        = $helper->requestToken($this->config->get('payment_accept_jawwalpay_api'));
        $integration_id     = $this->config->get('payment_accept_jawwalpay_int');
        $merchant_id        = $this->config->get('payment_accept_jawwalpay_merchant');
        $iframe_id          = $this->config->get('payment_accept_jawwalpay_iframe');
        $data['iframe_css'] = $this->config->get('payment_accept_jawwalpay_iframe_css');
        ...
        if ($customer) {
            ...
            $cards       = $this->db->query("SELECT * FROM `" . DB_PREFIX . "accept_jawwalpay_tokens` WHERE `customer_id` = '" . $customer_id . "'")->rows;
            ...
            if ($cards) {
                ...
                foreach ($cards as $card) {
                    ...
                    if (isset($acceptPaymentForSavedCards['token'])) {
                        ...
                        $data['iframes'][$i] = [
                            'url'          => 'https://checkout.jawwalpay.ps/api/acceptance/iframes/' . $iframe_id . '?payment_token=' . $keys[$i],
        ...
        $data['url'] = 'https://checkout.jawwalpay.ps/api/acceptance/iframes/' . $iframe_id . '?payment_token=' . $acceptPayment['token'];

        return $this->load->view('extension/payment/accept_jawwalpay', $data);
    }
    ...
    public function callback()
    {
        ...
        $hmac = $this->config->get('payment_accept_jawwalpay_hmac');
        ...
        if ($hash === $_REQUEST['hmac']) {
            if ($_SERVER['REQUEST_METHOD'] === 'POST') {
                if ($type == 'TRANSACTION') {
                    ...
                    if ($customer) {
                        $cards = $this->db->query("SELECT * FROM `" . DB_PREFIX . "accept_jawwalpay_tokens` WHERE `card_subtype` = '" . $data['card_subtype'] . "' AND `masked_pan` = '" . $data['masked_pan'] . "' AND `customer_id` = '" . (int) $customer['customer_id'] . "'")->rows;
                        if (!$cards) {
                            $this->db->query("INSERT INTO `" . DB_PREFIX . "accept_jawwalpay_tokens` SET `customer_id` = '" . (int) $customer['customer_id'] . "', `token` = '" . $data['token'] . "', `masked_pan` = '" . $data['masked_pan'] . "', `card_subtype` = '" . $data['card_subtype'] . "'");
                        } else {
                            $this->db->query("UPDATE `" . DB_PREFIX . "accept_jawwalpay_tokens` SET `customer_id` = '" . (int) $customer['customer_id'] . "', `token` = '" . $data['token'] . "', `masked_pan` = '" . $data['masked_pan'] . "', `card_subtype` = '" . $data['card_subtype'] . "' WHERE `card_subtype` = '" . $data['card_subtype'] . "' AND `masked_pan` = '" . $data['masked_pan'] . "' AND `customer_id` = '" . (int) $customer['customer_id'] . "'");
                        }
                    }
                }
            } else {
